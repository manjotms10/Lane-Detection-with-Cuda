...
#define checkCudaErrors(a)  if(a) {printf("\n");}
...
CannyEdgeDevice::CannyEdgeDevice(float *h_image, int width, int height) {
	...
	cudaMemcpy(this->d_image, h_image, sizeof(float) * width * height, cudaMemcpyHostToDevice);

	cudaMalloc((void**)&d_filterImage, sizeof(float) * width * height);
	cudaMalloc((void**)&d_gradientX, sizeof(float) * width * height);
	cudaMalloc((void**)&d_gradientY, sizeof(float) * width * height);
	cudaMalloc((void**)&d_gradientMag, sizeof(float) * width * height);
	cudaMalloc((void**)&d_nonMaxSup, sizeof(float) * width * height);
	cudaMalloc((void**)&d_highThreshHyst, sizeof(float) * width * height);
	cudaMalloc((void**)&d_lowThreshHyst, sizeof(float) * width * height);

	cudaMalloc((void**)&d_gaussianKernel, sizeof(float) * GAUSSIAN_KERNEL_SIZE * GAUSSIAN_KERNEL_SIZE);
	cudaMalloc((void**)&d_sobelKernelX, sizeof(float) * SOBEL_KERNEL_SIZE * SOBEL_KERNEL_SIZE);
	cudaMalloc((void**)&d_sobelKernelY, sizeof(float) * SOBEL_KERNEL_SIZE * SOBEL_KERNEL_SIZE);
...
CannyEdgeDevice::~CannyEdgeDevice() {
	...
	if (d_filterImage != NULL) cudaFree(d_filterImage);
	if (d_gradientX != NULL) cudaFree(d_gradientX);
	if (d_gradientY != NULL) cudaFree(d_gradientY);
	if (d_gradientMag != NULL) cudaFree(d_gradientMag);
	if (d_nonMaxSup != NULL) cudaFree(d_nonMaxSup);
	if (d_highThreshHyst != NULL) cudaFree(d_highThreshHyst);
	if (d_lowThreshHyst != NULL) cudaFree(d_lowThreshHyst);
	if (d_gaussianKernel != NULL) cudaFree(d_gaussianKernel);
	if (d_sobelKernelX != NULL) cudaFree(d_sobelKernelX);
	if (d_sobelKernelY != NULL) cudaFree(d_sobelKernelY);
}
...
void CannyEdgeDevice::performGaussianFiltering() {
	...
	Conv2D << <grid, block >> >(d_image, d_gaussianKernel, d_filterImage, width, height, GAUSSIAN_KERNEL_SIZE);
...
void CannyEdgeDevice::performImageGradientX() {
	...
	Conv2D << <dimGrid, dimBlock >> >(d_filterImage, d_sobelKernelX, d_gradientX, width, height, SOBEL_KERNEL_SIZE);
...
void CannyEdgeDevice::performImageGradientY() {
	...
	dim3 dimGrid(ceil((float)width / TILE_WIDTH), ceil((float)height / TILE_WIDTH));
	dim3 dimBlock(TILE_WIDTH, TILE_WIDTH, 1);
	Conv2D << <dimGrid, dimBlock >> >(d_filterImage, d_sobelKernelY, d_gradientY, width, height, SOBEL_KERNEL_SIZE);
